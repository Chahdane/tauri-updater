...
`delta-release` is file-in, file-out: previous installers plus the new installer
produce direct-to-current patches, the digests, the signature and the manifest.
It runs on CI once per release, never on a user's machine, and needs no network
access — which also makes it runnable by hand against local artifact files.
